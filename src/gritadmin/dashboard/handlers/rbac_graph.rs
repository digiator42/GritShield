...
pub async fn handle_rbac_dashboard(ctx: RequestContext) -> Response {
    // 1. Capture dynamic runtime inheritance maps
    let inheritance = &ctx.role_inheritance;

    // 2. Safely collect automatically discovered inventory endpoints
    // This draws directly from the AutoRoute struct filled by your #[controller] macro
    let mut route_capabilities = Vec::new();
    for route in crate::inventory::iter::<crate::routing::AutoRoute> {
        if route.required_role.is_some() || !route.path.contains("/admin/") {
            route_capabilities.push((
                format!("{:?}", route.method),
                route.path.to_string(),
                route.required_role.clone().unwrap_or_else(|| "Capability Secured").to_string()
            ));
        }
    }

    let markup = render_rbac_graph(inheritance, &route_capabilities);
                            ...
                            table class="w-full text-left font-mono text-xxs m-0 border-collapse" {
                                thead class="bg-gray-900 text-gray-400 uppercase tracking-wider" {
                                    tr {
                                        ...
                                        th class="p-2.5 font-semibold text-right" { "Structural Guard Assert" }
                                    }
                                }
