...
```rust
pub async fn dashboard_handler(ctx: RequestContext) -> Response {
    if ctx.session.is_none() {
        return Response::unauthorized("<h1>Access Denied</h1><p>Please log in first.</p>");
    }
    // ... load dashboard
}
```
...
```rust
pub async fn register_api(ctx: RequestContext) -> Response {
    // ... create user logic ...
    
    Response::created(&HashMap::from([
...
pub async fn admin_panel(ctx: RequestContext) -> Response {
    if user_role != "SuperAdmin" {
        return Response::forbidden(html! {
            div class="error-box p-6 bg-red-950 text-red-200 rounded-lg" {
                h1 class="text-xl font-bold" { "🔒 Insufficient Clearance" }
                p { "Your profile role scope does not grant access." }
            }
        });
    }
    // ... open panel
